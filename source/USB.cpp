...
void PuertoUSB::EnviarDatos_Respuesta() {
    ...
    if (MPUSBWrite(myOutPipe, TxBuffer, SendLength, &SentDataLength, SendDelay)) {
        if (MPUSBRead(myInPipe, RxBuffer, ExpectedReceiveLength, &ReceiveLength, ReceiveDelay)) {
            ;
        } else {
            CheckInvalidHandle();
        }
...
void PuertoUSB::soloEsperarRespuesta(unsigned int ExpectedLenght, char *puntero) {
    DWORD selection;
    DWORD ReceiveLength = 0;
    ...
    if (MPUSBRead(myInPipe, receive_buf, ExpectedReceiveLength, &ReceiveLength, ReceiveDelay)) {
        ;
    } else {
        CheckInvalidHandle();
    }
